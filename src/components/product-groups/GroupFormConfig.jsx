            ...
            <form onSubmit={onSubmit} className="space-y-5 w-full">
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="Ej: DERMO_PROMO_Q3"
                        className="w-full bg-white border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:outline-none focus:border-indigo-500 transition-all font-mono shadow-inner disabled:opacity-60"
                        required
                    />
                </div>
                ...
                <div className="flex items-center gap-3 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 shadow-inner">
                    ...
                    />
                    <label htmlFor="active" className="text-xs font-bold text-slate-600 cursor-pointer select-none">Grupo habilitado para Promociones</label>
                </div>

                {/* ASIGNACIÓN DE SKUS */}
                <div className="border-t border-slate-100 pt-4">
                    <label className="block text-[10px] font-bold uppercase tracking-wider text-slate-400 mb-2">
                        SKUs asignados a este Grupo ({formData.values.length})
                    </label>
                    <div className="min-h-36 bg-slate-50 rounded-xl p-3 border border-slate-200 flex flex-wrap gap-1.5 items-start max-h-64 overflow-y-auto custom-scrollbar shadow-inner">
                        {formData.values.length === 0 ? (
                            <span className="text-xs text-slate-400 font-medium italic m-auto text-center">Usa el buscador de la derecha para incluir códigos.</span>
                        ) : (
                            formData.values.map((sku, index) => (
                                <div key={`form-sku-${index}`} className="flex items-center gap-1.5 bg-white border border-slate-200 pl-2.5 pr-1 py-0.5 rounded-lg text-[11px] font-mono font-bold text-slate-700 hover:border-rose-300 transition-all shadow-sm">
                                    <span>{sku}</span>
                                    <button type="button" onClick={() => onRemoveSku(sku)} className="text-slate-400 hover:text-rose-600 p-0.5 rounded">
                                        <X size={12} />
                                    </button>
                                </div>
                            ))
